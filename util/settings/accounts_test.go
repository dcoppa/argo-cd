...
func TestAddAccount_AccountAdded(t *testing.T) {
	...
	cm, err := clientset.CoreV1().ConfigMaps("default").Get(context.Background(), mgr.GetArgoCDConfigMapName(), metav1.GetOptions{})
	assert.NoError(t, err)

	assert.Equal(t, cm.Data["accounts.test"], "login")
	assert.Equal(t, cm.Data["accounts.test.enabled"], "false")

	secret, err := clientset.CoreV1().Secrets("default").Get(context.Background(), mgr.GetArgoCDSecretName(), metav1.GetOptions{})
	assert.NoError(t, err)
