...
import (
	...
	"github.com/dcoppa/argo-cd/v2/util"
...
func (mgr *SettingsManager) GetArgoCDConfigMapName() string {
	return strings.Replace(mgr.namespace, "-ns-", "-cm-", 1)
}

func (mgr *SettingsManager) GetArgoCDSecretName() string {
	return strings.Replace(mgr.namespace, "-ns-", "-secret-", 1)
}
...
func (mgr *SettingsManager) updateSecret(callback func(*apiv1.Secret) error) error {
	...
	argoCDSecret, err := mgr.secrets.Secrets(mgr.namespace).Get(mgr.GetArgoCDSecretName())
	createSecret := false
	if err != nil {
...
func (mgr *SettingsManager) updateConfigMap(callback func(*apiv1.ConfigMap) error) error {
	...
	if err != nil {
		...
		argoCDCM = &apiv1.ConfigMap{
			ObjectMeta: metav1.ObjectMeta{
				Name: mgr.GetArgoCDConfigMapName(),
			},
		}
		createCM = true
	}
...
func (mgr *SettingsManager) getConfigMap() (*apiv1.ConfigMap, error) {
	...
	argoCDCM, err := mgr.configmaps.ConfigMaps(mgr.namespace).Get(mgr.GetArgoCDConfigMapName())
	if err != nil {
		return nil, err
	}
	if argoCDCM.Data == nil {
